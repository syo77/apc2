...
int main() {
  ...
  for(int lin = 0, linF = 10; lin < 10; lin++, linF--) {
    for (int col = 0, colF = 90; col < 10; col++, colF++) {

      *(pchar2 + (lin * 10 * 4) + 3) = 0; // R
      *(pchar2 + (lin * 10 * 4) + 2) = 0; // G
      *(pchar2 + (lin * 10 * 4) + 1) = 255; // B

      *(pchar2 + (linF * 10 * 4) - 2) = 0; // R
      *(pchar2 + (linF * 10 * 4) - 1) = 0; // G // Na verdade é R
      *(pchar2 + (linF * 10 * 4) - 0) = 255; // B

      *(pchar2 + (col * 4) + 3) = 0; // R
      *(pchar2 + (col * 4) + 2) = 0; // G
      *(pchar2 + (col * 4) + 1) = 255; // B

      *(pchar2 + (colF * 4) + 3) = 0; // R
      *(pchar2 + (colF * 4) + 2) = 0; // G
      *(pchar2 + (colF * 4) + 1) = 255; // B
  ...
  for (int lin = 0; lin < 10; lin++) {
      for (int col = 0; col <= lin; col++) {
          int pixelVerde = (col * 10 + lin) * 4;
          pchar3[pixelVerde + 3] = 0;   // R
          pchar3[pixelVerde + 2] = 255; // G
          pchar3[pixelVerde + 1] = 0;   // B
          pchar3[pixelVerde]  = 255;    // A
      }
  }
      
  // insira o seu código acima

  // Exercício 3 ARVORE INVERTIDA
  ...
  for (int lin = 0; lin < 10; lin++) {
    for (int col = 0; col < 10; col++) {
      if (col >= lin) {
        int pixelInv = (col * 10 + lin) * 4;
        pchar4[pixelInv + 3] = 255;
        pchar4[pixelInv + 2] = 0;
        pchar4[pixelInv + 1] = 0;
        pchar4[pixelInv]  = 255;
      }
